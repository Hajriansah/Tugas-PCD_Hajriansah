...
def process_image(method):
    ...
    if method == 'negative_transform':
        ...
        show_image(corrected_img, 510, 140, 'Hasil Metode Smoothing')
    elif method == 'brightness':
        corrected_img = brightness_correction(original_img)
        show_image(corrected_img, 720, 140, 'Hasil Metode Kecerahan')


# fungsi untuk menampilkan informasi pembuat program
def show_creator():
    creator_label = tk.Label(root, text='Nama : Hajriansah     |   NIM : F55121046   |   Kelas : B   |   Prodi : S1-Teknik Informatika')
